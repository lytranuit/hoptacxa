...
class NewsModel extends Model
{
        ...
        if ($type == "array" && !isset($data['id'])) {
            foreach ($data as &$row) {
                if (gettype($row) == "object") {
                    ...
                    //    $news_id = $row->id;
                    //    $builder = $this->db->table('htx_news_image')->join("htx_file", "htx_news_image.image_id = htx_file.id");
                    //    $row->image_other = $builder->where('news_id', $news_id)->get()->getResult();
                    //}
                    if (in_array("tags", $relation)) {
                        $news_id = $row->id;
                        $builder = $this->db->table('htx_news_tag')->join("htx_tag", "htx_news_tag.tag_id = htx_tag.id");
    ...
    public function get_news_related($id, $tags)
    {
        ...
        $builder = $this->db->table("htx_news");
        $builder->whereIn('id', function (BaseBuilder $builder) use ($tags) {
            return $builder->select('news_id')->from('htx_news_tag')->whereIn('tag_id', $tags);
        });
    ...
    public function get_news($tag_id = 0, $keyword = "", $offset = 0, $limit = 20)
    {
        ...
        if ($tag_id > 0) {
            $builder->whereIn('id', function (BaseBuilder $builder) use ($tag_id) {
                return $builder->select('news_id')->from('htx_news_tag')->where('tag_id', $tag_id);
            });
        }
        if ($keyword != "") {
            $builder->groupStart();
