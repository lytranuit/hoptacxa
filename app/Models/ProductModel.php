...
class ProductModel extends Model
{
            ...
            foreach ($data as &$row) {
                if (gettype($row) == "object") {
                    ...
                    if (in_array("image_other", $relation)) {
                        $product_id = $row->id;
                        $builder = $this->db->table('htx_product_image')->join("htx_file", "htx_product_image.image_id = htx_file.id");
                        $row->image_other = $builder->where('product_id', $product_id)->get()->getResult();
                    }
                    if (in_array("categories", $relation)) {
                        $product_id = $row->id;
                        $builder = $this->db->table('htx_product_category')->join("htx_category", "htx_product_category.category_id = htx_category.id");
    ...
    public function get_product_related($id, $categories)
    {
        ...
        $builder = $this->db->table("htx_product");
        $builder->whereIn('id', function (BaseBuilder $builder) use ($categories) {
            return $builder->select('product_id')->from('htx_product_category')->whereIn('category_id', $categories);
        });
    ...
    public function get_product($category_id = 0, $keyword = "", $offset = 0, $limit = 20)
    {
        ...
        if ($category_id > 0) {
            $builder->whereIn('id', function (BaseBuilder $builder) use ($category_id) {
                return $builder->select('product_id')->from('htx_product_category')->where('category_id', $category_id);
            });
        }
        if ($keyword != "") {
            $builder->groupStart();
