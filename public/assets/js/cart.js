...
$(document).ready(function() {
    ...
    $(document).on("change", ".number-widget .number", function() {
        if ($(this).autoNumeric("get") < 1) {
            $(this).val(1);
        }
    })
    $(document).on("click", ".number-widget .minus", function() {
    ...
    $(document).on("click", ".number-widget .plus", function() {
        let parent = $(this).parent();
        let numberEl = $(".number", parent);
        var amount = parseInt(numberEl.autoNumeric("get"));
        amount++;
...
function initCart() {
    ...
    $('.header-cart-icon').attr("data-icon-label", count);

    //if (count == 0) {
    //    $('.cart_quantity').addClass("d-none");
    //} else {
    //    $('.cart_quantity').removeClass("d-none");
    //}
    $.ajax({
        url: path + "ajax/cart",
        data: {},
